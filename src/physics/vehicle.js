 ...
 *
 * The drivetrain remains longitudinal, but the chassis owns planar velocity
 * and yaw rate independently from its visual heading. A wall impact therefore
 * changes momentum through an impulse instead of instantly rotating the car to
 * face a reflected direction.
 */
...
export class Vehicle {
  ...
  /**
   * Advance the simulation. The callback runs after every 5 ms body step so
   * collision detection and response use the physics timestep, not the render
   * frame rate.
   */
  update(dt, collisionStep = null) {
    ...
    while (this._accumulator >= FIXED_DT) {
      const previousPose = { x: this.x, z: this.z, heading: this.heading };
      this.step(FIXED_DT);
